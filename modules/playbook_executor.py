...
class PlaybookExecutor:
    ...
    def _execute_module(self, method: Dict, target: str) -> Dict:
        ...
        try:
            ...
            elif module_name == 'file_repair':
                # Run the file repair script
                import sys
                import os
                script_path = os.path.join(os.path.dirname(__file__), 'file_repair.py')
                result = subprocess.run(
                    [sys.executable, script_path, target],
                    capture_output=True,
                    text=True,
            ...
            elif module_name == 'network_extractor':
                # Run the network intelligence extractor
                import sys
                import os
                script_path = os.path.join(os.path.dirname(__file__), 'network_extractor.py')
                result = subprocess.run(
                    [sys.executable, script_path, target],
                    capture_output=True,
                    text=True,
            ...
            elif module_name == 'crypto_analyzer':
                # Run the crypto analyzer
                import sys
                import os
                script_path = os.path.join(os.path.dirname(__file__), 'crypto_analyzer.py')
                result = subprocess.run(
                    [sys.executable, script_path, target],
                    capture_output=True,
                    text=True,
